...
function getNextThemeColor() {
  const usedColorCount = getActiveRenters().length;
  return THEME_COLOR_PALETTE[usedColorCount % THEME_COLOR_PALETTE.length];
}
...
async function loadBusinessProfile() {
  ...
  if (!state.businessProfile) {
    openBusinessSettingsModal({ isFirstTimeSetup: true });
  }
}
...
function openAddRenterModal() {
  ...
  `,
    () => {
      document.getElementById("themeColor").value = getNextThemeColor();
...
function openBusinessSettingsModal({ isFirstTimeSetup = false } = {}) {
  ...
  openModal(
    ...
    <form id="businessSettingsForm" class="form-grid">
      ...
      <div><label for="businessLogoUrl">Business Logo URL</label><input id="businessLogoUrl" value="${safe(business.businessLogoUrl)}" /></div>
...
function setupMobileBottomNav() {
  mobileBottomNav?.querySelectorAll("button[data-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (action === "move") showToast("Move action is not configured for this view.");
...
settingsBtn?.addEventListener("click", () => openBusinessSettingsModal());
